...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  function buyItem(uint sku) 
    public payable forSale(sku) paidEnough(items[sku].price) checkValue(sku){
      //Needs 3 modifiers (check if item is for sale, buyer paid enough, check value after function is called to make sure buyer is refunded any excess
      //Transfer money to seller
      //address(uint160(msg.sender)).transfer(items[sku].price);
      (bool sent, bytes memory data) = items[sku].seller.call.value(items[sku].price)("");
      require(sent, "Failed to send Ether");
      //Set buyer as person who called this tx
      items[sku].buyer = msg.sender;
      //Set item's state to sold
